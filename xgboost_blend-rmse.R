...
#        }))
    ...
    #data<-as.matrix(data)
    model_num <-init
    single_model_metrics <- sapply(data, function(x){RMSE(x, reference) })
    sm_desc_ord  <-order(single_model_metrics, decreasing = FALSE)
    ...
    avg_gini <-RMSE(avg_prediction, reference)
    print("Initial gini: ")
    print(avg_gini)
    gini[1] = avg_gini
    ...
    for (i in 2:iter ) {
        ...
        metrics <- sapply(input, function(x) {
            if (runif(1)< sample) {
                RMSE((x + sums)/model_num, reference) 
            } else {
                100000000
            }
        })
        sel <- which.min(metrics)
...
write.csv(ensambleParams, file.path('target','ensamble-rmse.csv'), quote=FALSE)
